...
export async function listExamAnnouncements() {
    const pool = getPool();
    if (!pool) {
        return inMemoryExams;
    }
    try {
        const { rows } = await pool.query(`SELECT id, title, description, exam_date, posted_by, created_at
...
export async function createExamAnnouncement(input, postedBy) {
    const pool = getPool();
    if (!pool) {
        const exam = {
            id: nextExamId++,
    ...
    try {
        const { rows } = await pool.query(`INSERT INTO exam_announcements (title, description, exam_date, posted_by)
       VALUES ($1, $2, $3, $4)
       RETURNING id, title, description, exam_date, posted_by, created_at`, [input.title, input.description, input.examDate, postedBy ?? null]);
