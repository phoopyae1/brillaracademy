...
export async function getSemesterDate(semester) {
    const pool = getPool();
    if (!pool) {
        return null;
    }
    try {
        const { rows } = await pool.query(`SELECT id, semester, start_date, end_date, created_at, updated_at
       FROM semester_dates
...
export async function setSemesterDate(semester, startDate, endDate, updatedBy) {
    const pool = getPool();
    if (!pool) {
        throw new Error('Database connection not available.');
    }
    try {
        const { rows } = await pool.query(`INSERT INTO semester_dates (semester, start_date, end_date, created_by, updated_by, created_at, updated_at)
       ...
       SET start_date = EXCLUDED.start_date,
           ...
           updated_by = EXCLUDED.updated_by,
           updated_at = NOW()
       RETURNING id, semester, start_date, end_date, created_at, updated_at`, [semester, startDate, endDate, updatedBy ?? null]);
        const row = rows[0];
        return {
            id: row.id,
            semester: row.semester,
...
export async function isRegistrationPeriodOpen(semester) {
    const result = await getRegistrationStatus(semester);
    return result.open;
}
export async function getRegistrationStatus(semester) {
    const pool = getPool();
    if (!pool) {
    ...
    try {
        ...
        const semesterDateRows = await pool.query(`SELECT start_date, end_date
       FROM semester_dates
       WHERE semester = $1`, [semester]);
        ...
        const { rows } = await pool.query(`SELECT closes_at, status
