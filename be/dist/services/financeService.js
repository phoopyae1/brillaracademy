...
export async function listFeePayments() {
    ...
    try {
        const { rows } = await pool.query(`SELECT id, student_id, amount, description, status, received_by, received_at, due_date
       FROM fee_payments
...
export async function listStudentFeePayments(studentId) {
    const pool = getPool();
    if (!pool) {
    ...
    try {
       ...
       FROM fee_payments
       WHERE student_id = $1
       ORDER BY received_at DESC`, [studentId]);
        return rows.map(normalizeFeePayment);
    }
    catch (error) {
...
export async function recordFeePayment(input, receivedBy) {
    const pool = getPool();
    ...
    if (!pool) {
        ...
        const existingFee = inMemoryFeePayments.find((fee) => fee.studentId === input.studentId &&
            fee.status === 'pending' &&
    ...
    try {
        ...
        if (description) {
            ...
            existingFee = await pool.query(`SELECT id, amount FROM fee_payments 
         WHERE student_id = $1 AND status = 'pending' 
         AND description = $2
         AND ABS(amount - $3) < 0.01
         ORDER BY id DESC
         LIMIT 1`, [input.studentId, description, input.amount]);
            ...
            if (existingFee.rows.length === 0) {
                existingFee = await pool.query(`SELECT id, amount, description FROM fee_payments 
           WHERE student_id = $1 AND status = 'pending' 
           AND ABS(amount - $2) < 0.01
           AND (description ILIKE $3 OR $3 ILIKE '%' || description || '%')
           ORDER BY id DESC
           LIMIT 1`, [input.studentId, input.amount, `%${description}%`]);
            }
        }
        ...
        if (!existingFee || existingFee.rows.length === 0) {
         ...
         AND ABS(amount - $2) < 0.01
         ORDER BY id DESC
         LIMIT 1`, [input.studentId, input.amount]);
        }
        if (existingFee.rows.length > 0) {
            // Update existing fee to paid
            const { rows } = await pool.query(`UPDATE fee_payments 
