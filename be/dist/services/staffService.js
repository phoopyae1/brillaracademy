...
export async function listStaff() {
    ...
    try {
        const { rows } = await pool.query(`SELECT id, display_name, email, role, created_at
       FROM staff_accounts
...
export async function findStaffByEmail(email) {
    ...
    try {
        const { rows } = await pool.query(`SELECT id, display_name, email, role, created_at, password_hash
       FROM staff_accounts
       WHERE email = $1`, [email]);
        if (!rows.length) {
            return null;
...
export async function createStaffAccount(displayName, email, password, role, createdBy) {
    const pool = getPool();
    ...
    try {
        const { rows } = await pool.query(`INSERT INTO staff_accounts (display_name, email, password_hash, role, created_by)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, display_name, email, role, created_at`, [displayName, email, hashedPassword, role, createdBy ?? null]);
...
export async function ensureInitialItAdmin() {
