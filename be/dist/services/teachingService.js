...
import { fallbackTeachingAssignments, fallbackTeacherRosters, fallbackTeacherFocusTags } from './fallbackData.js';
...
export async function listTeachingAssignments() {
    ...
    try {
        const { rows } = await pool.query(`SELECT id, teacher_id, classroom_id, course_code, course_title, weekday, start_time, end_time, student_group, major_focus, semester, assigned_by, assigned_at
...
export async function listTeachingAssignmentsForTeacher(teacherId) {
    const pool = getPool();
    if (!pool) {
    ...
    try {
       ...
       FROM teaching_assignments
       WHERE teacher_id = $1
       ORDER BY start_time ASC`, [teacherId]);
        const assignments = rows.map(normalizeAssignment);
        assignments.forEach((assignment) => {
            registerSubjectForMajor(assignment.majorFocus, assignment.courseTitle);
        });
...
async function enrollStudentsInAssignment(assignment, input, pool, assignedBy) {
    ...
    if (relevantStudents.length === 0) {
        ...
        console.warn(`[enrollStudentsInAssignment] ⚠ No students found with major "${assignment.majorFocus}" for assignment "${input.courseTitle}". ` +
    ...
    for (const student of relevantStudents) {
        ...
        const existingRoster = await pool.query(`SELECT id FROM teacher_rosters WHERE teacher_id = $1 AND course_code = $2 AND student_id = $3`, [input.teacherId, input.courseCode, student.id]);
        if (existingRoster.rows.length === 0) {
         ...
         VALUES ($1, $2, $3, $4, 'enrolled')`, [input.teacherId, input.courseCode, input.courseTitle, student.id]);
            ...
            const existingReg = await pool.query(`SELECT id FROM class_registrations WHERE student_id = $1 AND class_name = $2`, [student.id, input.courseTitle]);
           ...
           VALUES ($1, $2, $3, 'registered', $4, $5, $6, NOW())`, [student.id, input.courseTitle, instructor, input.semester ?? '1/2026', credits, assignedBy ?? null]);
            ...
            const existingTimetable = await pool.query(`SELECT id FROM timetables WHERE student_id = $1 AND subject = $2 AND weekday = $3`, [student.id, input.courseTitle, input.weekday]);
           ...
           VALUES ($1, $2, $3, $4, $5, 
             (SELECT location FROM classrooms WHERE id = $6))`, [student.id, input.weekday, input.startTime, input.endTime, input.courseTitle, input.classroomId]);
            }
            // Create fee payment (only if metadata exists to get accurate credit amount)
            if (metadata) {
                const existingFee = await pool.query(`SELECT id FROM fee_payments WHERE student_id = $1 AND description = $2`, [student.id, `${input.courseTitle} - Registration Fee`]);
...
async function validateClassroomMajorMatch(classroomId, majorFocus, pool) {
    if (!pool) {
        return true; // Skip validation in in-memory mode
    }
    try {
        const { rows } = await pool.query(`SELECT resources FROM classrooms WHERE id = $1`, [classroomId]);
        if (rows.length === 0) {
        ...
        const hasMatchingMajor = resources.some((resource) => {
        ...
        if (!hasMatchingMajor) {
            console.warn(`[TeachingService] ⚠️ Classroom ${classroomId} does not support major "${majorFocus}". ` +
                `Available majors in classroom resources: ${resources
                    .filter((r) => typeof r === 'string' && /Major:/i.test(r))
                    .map((r) => r.replace(/^\s*Major:\s*/i, ''))
                    .join(', ') || 'none'}`);
        }
...
export async function assignTeacherToClassroom(input, assignedBy) {
    ...
    if (input.majorFocus && input.classroomId) {
        ...
        if (!isValid) {
            throw new Error(`Cannot assign ${input.majorFocus} class to this classroom. ` +
...
export async function syncEnrollmentsForAssignment(assignmentId) {
    ...
    try {
      ...
      `);
        ...
        let rows;
        try {
            const result = await pool.query(`SELECT id, teacher_id, classroom_id, course_code, course_title, weekday, start_time, end_time, student_group, major_focus, semester
       FROM teaching_assignments
       WHERE id = $1`, [assignmentId]);
            ...
            if (queryError?.code === '42703' && queryError?.message?.includes('semester')) {
                ...
                const result = await pool.query(`SELECT id, teacher_id, classroom_id, course_code, course_title, weekday, start_time, end_time, student_group, major_focus
           FROM teaching_assignments
           WHERE id = $1`, [assignmentId]);
        ...
        for (const student of relevantStudents) {
            const existingRoster = await pool.query(`SELECT id FROM teacher_rosters WHERE teacher_id = $1 AND course_code = $2 AND student_id = $3`, [assignment.teacherId, assignment.courseCode, student.id]);
            if (existingRoster.rows.length === 0) {
                // Add to teacher roster
                await pool.query(`INSERT INTO teacher_rosters (teacher_id, course_code, course_title, student_id, status)
           VALUES ($1, $2, $3, $4, 'enrolled')`, [assignment.teacherId, assignment.courseCode, assignment.courseTitle, student.id]);
                // Get course metadata
                const { getCourseMetadata } = await import('../utils/majors.js');
                ...
                const existingReg = await pool.query(`SELECT id FROM class_registrations WHERE student_id = $1 AND class_name = $2`, [student.id, assignment.courseTitle]);
                if (existingReg.rows.length === 0) {
                    await pool.query(`INSERT INTO class_registrations (student_id, class_name, instructor, status, semester, credits, confirmed_by, registered_at)
             VALUES ($1, $2, $3, 'registered', $4, $5, NULL, NOW())`, [student.id, assignment.courseTitle, instructor, assignment.semester ?? '1/2026', credits]);
                    enrolled++;
                ...
                // Create timetable entry
                const existingTimetable = await pool.query(`SELECT id FROM timetables WHERE student_id = $1 AND subject = $2 AND weekday = $3`, [student.id, assignment.courseTitle, assignment.weekday]);
                if (existingTimetable.rows.length === 0) {
                    // Get classroom location first
                    const classroomResult = await pool.query(`SELECT location FROM classrooms WHERE id = $1`, [assignment.classroomId]);
                    const classroomLocation = classroomResult.rows.length > 0
                        ? classroomResult.rows[0].location
                        : 'TBA';
                    await pool.query(`INSERT INTO timetables (student_id, weekday, start_time, end_time, subject, location)
             VALUES ($1, $2, $3, $4, $5, $6)`, [student.id, assignment.weekday, assignment.startTime, assignment.endTime, assignment.courseTitle, classroomLocation]);
                }
            }
            else {
                skipped++;
...
export async function listTeacherRosters(teacherId) {
    const pool = getPool();
    if (!pool) {
        return inMemoryRosters.filter((roster) => roster.teacherId === teacherId);
    }
    try {
        const { rows } = await pool.query(`SELECT id, teacher_id, course_code, course_title, student_id, status
...
export function getTeacherFocusTags(teacherId) {
...
export async function buildTeacherDashboard(teacherId) {
    ...
    const schedule = sortSchedule(assignments.map((assignment) => {
...
//# sourceMappingURL=teachingService.js.map