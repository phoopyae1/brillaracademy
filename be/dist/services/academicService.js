...
function normalizeSemesterGpa(row) {
    return {
        id: row.id,
        studentId: row.student_id,
        semester: row.semester,
...
function normalizeRegistrationWindow(row) {
    return {
        id: row.id,
        semester: row.semester,
        status: row.status,
...
export async function listStudentGrades(studentId) {
    ...
    try {
        ...
        const { rows } = await pool.query(`SELECT id, student_id, course_code, course_title, semester, grade, credits, recorded_by, recorded_at
       FROM grade_records
       WHERE student_id = $1
       ORDER BY semester DESC, course_code ASC`, [studentId]);
        console.log(`[AcademicService] Found ${rows.length} grade record(s) for student ${studentId} in database`);
        if (rows.length > 0) {
            console.log(`[AcademicService] Sample grade records:`, rows.slice(0, 3).map((r) => ({
                id: r.id,
...
export async function listTeacherGrades(teacherId) {
    const pool = getPool();
    if (!pool) {
    ...
    try {
       ...
       FROM grade_records
       WHERE recorded_by = $1
       ORDER BY recorded_at DESC NULLS LAST, semester DESC`, [teacherId]);
...
export async function listStudentSemesterGpa(studentId) {
    const pool = getPool();
    if (!pool) {
    ...
    try {
        ...
        const { rows } = await pool.query(`SELECT id, student_id, semester, gpa
       FROM semester_gpa
       WHERE student_id = $1
       ORDER BY semester DESC`, [studentId]);
        console.log(`[AcademicService] Found ${rows.length} GPA record(s) for student ${studentId} in database`);
        if (rows.length > 0) {
            console.log(`[AcademicService] GPA records:`, rows.map((r) => ({
                semester: r.semester,
                gpa: r.gpa
            })));
        }
        ...
        if (rows.length === 0) {
            ...
            const semestersResult = await pool.query(`SELECT DISTINCT semester
         FROM grade_records
         WHERE student_id = $1 AND credits > 0`, [studentId]);
            if (semestersResult.rows.length > 0) {
                ...
                const updatedRows = await pool.query(`SELECT id, student_id, semester, gpa
           FROM semester_gpa
           WHERE student_id = $1
           ORDER BY semester DESC`, [studentId]);
...
export function findCourseOffering(semester, courseCode) {
 ...
 */
async function calculateAndUpdateSemesterGPA(studentId, semester) {
    const pool = getPool();
    if (!pool) {
...
export async function recordStudentGrade(input, recordedBy) {
    const pool = getPool();
    const recordedAt = new Date().toISOString();
    if (!pool) {
        // Check if grade already exists in memory
        const existingIndex = inMemoryGrades.findIndex((g) => g.studentId === input.studentId && g.courseCode === input.courseCode && g.semester === input.semester);
        if (existingIndex >= 0) {
    ...
    try {
        ...
        const existingGradesResult = await pool.query(`SELECT id, grade, credits, recorded_by, recorded_at
       FROM grade_records
       WHERE student_id = $1 AND course_code = $2 AND semester = $3
       ORDER BY recorded_at DESC`, [input.studentId, input.courseCode, input.semester]);
        let savedGrade;
        if (existingGradesResult.rows.length > 0) {
            ...
            const updateResult = await pool.query(`UPDATE grade_records
         ...
         RETURNING id, student_id, course_code, course_title, semester, grade, credits, recorded_by, recorded_at`, [
           ...
           WHERE student_id = $1 AND course_code = $2 AND semester = $3 AND id != $4`, [input.studentId, input.courseCode, input.semester, mostRecentId]);
                const deletedCount = deleteResult.rowCount || duplicateCount - 1;
            ...
            const insertResult = await pool.query(`INSERT INTO grade_records (student_id, course_code, course_title, semester, grade, credits, recorded_by, recorded_at)
