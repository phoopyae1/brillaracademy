...
export async function listStudentGrades(studentId) {
    ...
    try {
        const { rows } = await pool.query(`SELECT id, student_id, course_code, course_title, semester, grade, credits
       FROM grade_records
       WHERE student_id = $1
...
export async function recordStudentGrade(input, recordedBy) {
    ...
    try {
        const { rows } = await pool.query(`INSERT INTO grade_records (student_id, course_code, course_title, semester, grade, credits, recorded_by, recorded_at)
