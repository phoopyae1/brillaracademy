...
import { getCurrentSemester, getRegistrationStatus, getSemesterDate } from '../services/systemService.js';
function formatTimeToSingapore(time) {
    if (!time) {
        return null;
    }
    try {
...
function formatDateTimeToSingapore(isoDateTime) {
    if (!isoDateTime) {
        return null;
    }
    try {
 ...
 */
router.post('/student-registrations', requireStudent(), async (req, res) => {
    try {
        const { studentId } = req.body;
        if (!studentId || !Number.isFinite(Number(studentId))) {
            return res.status(400).json({ error: 'Invalid student ID in request body.' });
       ...
       ORDER BY cr.registered_at DESC`, [id]);
        const detailedRegistrations = rows.map((row) => {
 ...
 */
router.post('/announcements-events', requireStudent(), async (req, res) => {
    try {
        const { studentId } = req.body;
        const pool = getPool();
        if (!pool) {
 ...
 * Request body: { studentId: number }
 */
router.post('/admission-fees', requireStudent(), async (req, res) => {
    try {
        const { studentId } = req.body;
        if (!studentId || !Number.isFinite(Number(studentId))) {
            return res.status(400).json({ error: 'Invalid student ID in request body.' });
       ...
       FROM fee_payments
       WHERE student_id = $1
       ORDER BY received_at DESC NULLS LAST, due_date ASC NULLS LAST`, [id]);
                return rows;
            }
            catch (error) {
                if (error?.code === '42703') {
        ...
        const subjectRows = await (async () => {
           ...
           ORDER BY cr.registered_at DESC`, [id]);
             ...
             FROM class_registrations cr
             WHERE cr.student_id = $1
             ORDER BY cr.id DESC`, [id]);
                    return rows;
                }
                throw error;
            }
        })();
        const subjectFees = subjectRows.map((row) => {
        ...
        const otherFees = paymentRows
            .filter((row) => !matchedPaymentIds.has(row.id))
            .map((row) => {
        ...
        const applyPaymentStatus = (fees) => fees.map((fee) => {
 ...
 */
router.post('/assignments', requireStudent(), async (req, res) => {
    try {
        const { studentId } = req.body;
        if (!studentId || !Number.isFinite(Number(studentId))) {
            return res.status(400).json({ error: 'Invalid student ID in request body.' });
 ...
 */
router.post('/grades', requireStudent(), async (req, res) => {
    try {
        const { studentId } = req.body;
        if (!studentId || !Number.isFinite(Number(studentId))) {
            return res.status(400).json({ error: 'Invalid student ID in request body.' });
       ...
       ORDER BY gr.semester DESC, gr.course_code ASC`, [id]);
        const totalCredits = rows.reduce((sum, row) => sum + Number(row.credits ?? 0), 0);
        const totalPoints = rows.reduce((sum, row) => sum + gradeToPoint(row.grade) * Number(row.credits ?? 0), 0);
        const totalGpa = totalCredits > 0 ? Math.round((totalPoints / totalCredits) * 100) / 100 : null;
        return res.json({
            grades: rows.map((row) => ({
                id: row.id,
                studentId: row.studentId,
 ...
 */
router.post('/register-course', requireStudent(), async (req, res) => {
    try {
        ...
        if (rows.length === 0) {
            ...
            if (courseRows.length === 0) {
                ...
                let otherSemesterQuery;
                let otherSemesterParams;
                if (hasCourseCode) {
                ...
                if (otherSemesterRows.length > 0) {
                    const otherSemesters = otherSemesterRows.map((r) => r.semester).join(', ');
            ...
            const availableSlots = courseRows.map((r) => ({
                weekday: r.weekday,
        ...
        const { rows: existingRoster } = await pool.query(`SELECT tr.id,
              to_char(ta.start_time, 'HH24:MI') AS start_time,
              ta.weekday
       ...
       LIMIT 1`, [id, course.course_code] // Use the course code from the found course
        );
        ...
        const { rows: existingTimetable } = await pool.query(`SELECT id
       FROM timetables
       WHERE student_id = $1
         AND LOWER(TRIM(subject)) = LOWER(TRIM($2))
         AND to_char(start_time, 'HH24:MI') = $3
         AND LOWER(TRIM(weekday)) = LOWER(TRIM($4))
       LIMIT 1`, [id, course.course_title, course.start_time, course.weekday]);
        ...
        try {
            const enrollment = await registerStudentForClassroom(id, course.classroom_id, course.course_code, // Use the course code from the found course
            ...
            try {
                ...
                const existingReg = await pool.query(`SELECT id FROM class_registrations 
           WHERE student_id = $1 AND LOWER(TRIM(class_name)) = LOWER(TRIM($2))`, [id, course.course_title]);
                if (existingReg.rows.length === 0) {
                    ...
                    await pool.query(`INSERT INTO class_registrations 
             (student_id, class_name, instructor, status, semester, credits, confirmed_by, registered_at)
             VALUES ($1, $2, $3, 'registered', $4, $5, NULL, NOW())`, [id, course.course_title, course.teacher_name || 'TBA', semester, credits]);
 ...
 */
router.post('/available-courses', requireStudent(), async (req, res) => {
    try {
        const { studentId } = req.body;
        if (!studentId || !Number.isFinite(Number(studentId))) {
            return res.status(400).json({ error: 'Invalid student ID in request body.' });
        ...
        const { rows } = await pool.query(`SELECT DISTINCT ON (ta.course_code, ta.weekday, ta.start_time, ta.teacher_id)
              ta.course_code AS "courseCode",
       ...
       ORDER BY ta.course_code, ta.weekday, ta.start_time, ta.teacher_id, ta.id ASC`, [currentSemester, normalizedMajor]);
        ...
        if (filteredCourses.length > 0) {
            try {
                ...
                const { rows: countRows } = await pool.query(`SELECT ta.course_code, ta.weekday, to_char(ta.start_time, 'HH24:MI') AS start_time, 
           ...
           GROUP BY ta.course_code, ta.weekday, ta.start_time`, [currentSemester, courseCodes]);
