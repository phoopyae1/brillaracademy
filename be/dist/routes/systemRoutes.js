...
import { setSystemSetting, getCurrentSemester, getSemesterDate, setSemesterDate, listSemesterDates } from '../services/systemService.js';
const router = Router();
...
router.put('/settings/current-semester', requireStaff(['IT_ADMIN', 'STUDENT_ADMIN']), async (req, res) => {
...
router.put('/settings/semester-dates', requireStaff(['IT_ADMIN', 'STUDENT_ADMIN']), async (req, res) => {
    ...
    try {
        const semesterDate = await setSemesterDate(parseResult.data.semester, parseResult.data.startDate, parseResult.data.endDate, req.staff?.id);
        res.json({
