 ...
 */
export async function listStudentAssignments(studentId: number): Promise<StudentAssignment[]> {
  const pool = getPool();
 ...
 */
export async function listTeacherAssignments(teacherId: number, courseCode?: string): Promise<StudentAssignment[]> {
  const pool = getPool();
 ...
 */
export async function createAssignment(input: CreateAssignmentInput): Promise<StudentAssignment> {
  const pool = getPool();
  ...
  try {
    const { rows } = await pool.query(
      `INSERT INTO student_assignments (teacher_id, course_code, course_title, title, description, due_date, max_points, assignment_type)
 ...
 */
export async function updateAssignment(input: UpdateAssignmentInput): Promise<StudentAssignment> {
  const pool = getPool();
...
function normalizeAssignment(row: any): StudentAssignment {
  return {
    id: row.id,
