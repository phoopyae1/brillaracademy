...
async function calculateGPAForAllStudents() {
  ...
  try {
    ...
    const studentSemestersResult = await pool.query(
    ...
    for (const row of studentSemestersResult.rows) {
      ...
      try {
        ...
        const existingGpaResult = await pool.query(
          `SELECT id, gpa FROM semester_gpa
           WHERE student_id = $1 AND semester = $2`,
          [studentId, semester]
        );
        ...
        if (existingGpaResult.rows.length > 0) {
          ...
          // Update existing GPA
          await pool.query(
            `UPDATE semester_gpa SET gpa = $1 WHERE student_id = $2 AND semester = $3`,
            [roundedGpa, studentId, semester]
          );
          updatedCount++;
