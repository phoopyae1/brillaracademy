...
async function cleanupDuplicateGrades() {
  ...
  try {
    ...
    const duplicatesResult = await pool.query(
      `SELECT student_id, course_code, semester, COUNT(*) as count
       FROM grade_records
    ...
    for (const dup of duplicatesResult.rows) {
      ...
      const deleteIds = gradesResult.rows.slice(1).map((r: any) => r.id);
      ...
      const deletedCount = (deleteResult as any).rowCount || deleteIds.length;
